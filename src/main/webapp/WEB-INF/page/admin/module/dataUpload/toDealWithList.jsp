...
<div class="bjui-pageHeader">
    <form id="pagerForm" data-toggle="ajaxsearch" action="${pageContext.request.contextPath}/dataUpload/toDealWithList" method="post">
...
<div class="bjui-pageContent">
    <table class="table table-bordered table-hover table-striped table-top" data-selected-multi="true">
        ...
        <tr>
            ...
            <th class="project">指派的数据上传者</th>
            <th class="project">数据上传模块状态</th>
        ...
        <c:forEach var="list" items="${dealWithList.content}" varStatus="i">
            <tr >
                ...
                <td class="project">${list.dataUpload.dataUploadToAppointPeople}</td>
                <td class="project">${list.dataUpload.getDataUploadToAppointDateStr()}</td>
                <td class="project">${list.dataUpload.dataUploadPeople}</td>
                ...
                <td class="project">
                    <c:if test="${list.dataUpload.is_dataUploadPeople!=1}">
                        <a href="${pageContext.request.contextPath}/dataUpload/toAccept?id=${list.id}" class="btn btn-default" data-toggle="doajax" data-confirm-msg="确定要接受处理吗？">接受</a>
                ...
                <td class="project">
                    <a href="${pageContext.request.contextPath}/dataUpload/toDealWith?id=${list.id}" class="btn btn-info" data-toggle="dialog" data-width="500" data-height="300" data-id="dialog-mask" data-mask="true" <c:if test="${list.dataUpload.is_dataUploadPeople!=1}">disabled="disabled" </c:if> >处理</a>
                    <a href="${pageContext.request.contextPath}/dataUpload/toDealWithFinish?id=${list.id}" class="btn btn-default" data-toggle="doajax" data-confirm-msg="确定要处理完成吗？" <c:if test="${list.dataUpload.is_dataUploadPeople!=1}">disabled="disabled" </c:if>>处理完成</a>
                </td>
            </tr>
        </c:forEach>
