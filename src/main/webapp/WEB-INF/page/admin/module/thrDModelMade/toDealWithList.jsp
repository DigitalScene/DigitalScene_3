...
<div class="bjui-pageHeader">
    <form id="pagerForm" data-toggle="ajaxsearch" action="${pageContext.request.contextPath}/admin/thrDModelMade/toDealWithList" method="post">
    ...
    <table class="table table-bordered table-hover table-striped table-top" data-selected-multi="true">
        ...
        <c:forEach var="list" items="${dealWithList.content}" varStatus="i">
            <tr >
                ...
                <td class="project">
                    <c:if test="${list.thrDModelMade.is_thrDModelMadePeople!=1}">
                        <a href="${pageContext.request.contextPath}/admin/thrDModelMade/toAccept?id=${list.id}" class="btn btn-default" data-toggle="doajax" data-confirm-msg="确定要接受处理吗？">接受</a>
                        <a href="${pageContext.request.contextPath}/admin/common/module/toReject?id=${list.id}&type=thrDModelMade" class="btn btn-red" data-toggle="dialog" data-width="680" data-height="240" data-id="dialog-mask" data-mask="true">拒绝</a>
                    </c:if>
                    <c:if test="${list.thrDModelMade.is_thrDModelMadePeople==1}">
                        已接受
                    </c:if>
                ...
                <td class="project">
                    <a href="${pageContext.request.contextPath}/admin/thrDModelMade/toDealWith?thrDModelMadeId=${list.thrDModelMade.id}" class="btn btn-info" data-toggle="dialog" data-width="500" data-height="300" data-id="dialog-mask" data-mask="true" <c:if test="${list.thrDModelMade.is_thrDModelMadePeople!=1}">disabled="disabled" </c:if> >处理</a>
                    <a href="${pageContext.request.contextPath}/admin/thrDModelMade/toDealWithFinish?id=${list.id}" class="btn btn-default" data-toggle="doajax" data-confirm-msg="确定要处理完成吗？" <c:if test="${list.thrDModelMade.is_thrDModelMadePeople!=1}">disabled="disabled" </c:if>>处理完成</a>
                </td>
            </tr>
        </c:forEach>
